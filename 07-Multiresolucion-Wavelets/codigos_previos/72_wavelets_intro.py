...
def  agrupar(A, f):
    ...
    for p in range(0,4):
        ...
        if (p==0 and f>2):
            ...
            S[np.mod(p,2):n:2,(p>1):n:2]= agrupar(tmp, np.int(f/2))
        else:
            # otros cuadrantes
...
def  desagrupar(A, m, nse):
    ...
    for p in range(0,4):
        ...
        if (p==0 and m>2):
            #cuadrante PPAL
            S[ V[p]:V[p]+delta, H[p]:H[p]+delta]= desagrupar(tmp, np.int(m/2), nse)
        else:
            # otros cuadrantes
...
iter=8         #numero de subdivisiones (máx 8 niveles)
m=2**iter       #niveles
...
fig = plt.figure()
plt.subplot(121)
plt.imshow(enc, cmap= cm.hsv)
plt.axis('off')
plt.title('Codificación de Haar')

plt.subplot(122)
plt.imshow(grp, cmap= cm.hsv)
plt.axis('off')
